...
| REST API | `app/rest/app.py` (FastAPI wrapper) | порт 8000, `uv run --extra http uvicorn app.rest.app:api` |
...
**Причина**: REST API (`app/rest/app.py`) не запущен на порту 8000.

**Проверка**: `lsof -i :8000` или `curl -sf http://localhost:8000/api/health`.

**Фикс**:
```bash
nohup uv run --extra http uvicorn app.rest.app:api --host 0.0.0.0 --port 8000 > /tmp/dj-rest-api.log 2>&1 &
...
- Не трогаешь backend Python (только `app/rest/app.py` если там wiring баг).
