...
> cutover. Остатки `dj-loop` systemd unit на VM не поддерживаются —
> для расширения библиотеки используй MCP v1 surface через Claude
> Code `/loop` или ad-hoc вызовы prompt'ов.
...
3. Сопоставь с known issues: numba SEGV (см. `.claude/rules/audio.md` → Numba/librosa version pinning), BrokenProcessPool deadlock, Supabase idle disconnect (5 мин timeout).
