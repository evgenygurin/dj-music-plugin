...
- **Download:** `entity_create(entity="audio_file", data={"track_ids": [...]})`
  ...
  `DjLibraryItem`. `skip_existing` проверяет строку в БД **и** наличие
  файла на диске: если строка есть, а файл стёрт (например, зачистка
  `/tmp`) — handler перекачивает MP3 и обновляет ту же строку
  (`refreshed_stale_row: true` в ответе), дубликат не создаётся. Повторный
  вызов на скачанный, но не закоммиченный файл (после MCP-таймаута)
  по-прежнему перекачивает заново — строки в БД нет.
