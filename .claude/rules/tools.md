...
globs: src/dj_music/tools/**/*.py
---

# MCP Tools

- Use standalone `@tool` decorator from `fastmcp` (FileSystemProvider auto-discovers, scans subpackages recursively)
- **Tag and annotation constants**: import from `dj_music.tools._shared` — never hardcode literals
...
- YM tools live in `src/dj_music/tools/yandex/` (one file per entity), not in a flat `ym.py`
...
- `get_track` / `TrackService.search` распознают `ym:12345` / `YM:12345` префикс и резолвят через `track_external_ids` (`yandex_music`). Plain text query — fallback по title/artist. Implementation в `src/dj_music/services/track_service.py:_extract_ym_id`.
