...
- Для проверки runtime используй актуальный CLI-паттерн FastMCP 3:
  `fastmcp call <target> <tool_name> --input-json ... --json` или, для stdio,
  `fastmcp call --command <server-command> --target <tool> ...` если так
  устроен локальный runner. Структурный результат парси из JSON/.data, а не
  из человекочитаемого текста.
