...
| `ui_control_center` | version_id | Heading + Row[Metric×N] library/set/version overview + pipeline buttons (Analyze+QA·Render·Diagnose·Build/Reorder·Analyze→L5·conditional Sync-diff→YM) | `ControlCenterFallback` |
...
`ui_control_center` is the combined library + set/version + render-pipeline entry panel: it composes the library overview, the current set/version summary, and the pipeline controls in one view. Phase 2 adds Build/Reorder (hidden `act_build` — re-optimize + persist a new set_version), Analyze→L5 (hidden `act_l5_set`, `task=True` — batched download + L5 reanalyze, progress via `RENDER_JOBS`), and a conditional Sync-diff→YM button (shown only when `DjSet.source_playlist_id` is set; calls `playlist_sync(direction="diff")`). Buttons refresh through the dedicated hidden `control_center_panel` helper. All four helpers (`render_studio_panel`, `control_center_panel`, `act_build`, `act_l5_set`) are `visibility=["app"]` — registered but hidden from the model; model-visible count stays 25.
