...
| `key_confidence` | chromagram template-match strength | *should* gate how hard Camelot penalises — **but UNUSED by the live scorer** | **null/miscal** + **gap** (loaded, referenced by zero scoring fns) |
| `atonality` | bool = normalized `chroma_entropy > 0.92` | "relax the key rules" permission — atonal tracks are universal connectors. **98.7% of our library is atonal** | **null/miscal** + **gap** (UNUSED; the ≥5 Camelot hard-reject fires regardless → false-reject risk) |
| `chroma_entropy` | Shannon entropy of 12-bin chroma, **normalized [0,1]** (not raw bits) | harmonic busyness; 30% of vocals stem via `1−|d|/3.0` | **null/miscal** (0.96/0.98/0.99 near-constant; **+ a `/3.0` scale bug** — see Known code quirks) |
...
These are documented here and (where safe) fixed/commented in code; the risky scoring changes are flagged for review rather than applied blind.

- **`chroma_entropy /3.0` scale bug** (`neural_mix.score_vocal_compat` + `bulk_scorer`): the feature is normalized to [0,1] but the proximity divides by 3.0 (the old raw-bits scale), so the term can never drop below 0.667. Near-zero practical effect on *this* library (chroma is near-constant) but wrong in general. Fixed → `/1.0` (golden snapshots regenerated).
- **`key_confidence` + `atonality` are loaded but UNUSED by the scorer**; the Camelot distance ≥5 hard-reject is unconditional (blind to confidence/atonality/HNR). On a 98.7%-atonal library this risks **false hard-rejects**. Phase-2: skip/relax the hard reject when both tracks are atonal or low-confidence. (Not applied — changes the gate; needs validation.)
- **`dissonance_mean` penalty (`>0.4` both)** fires near-uniformly (data ≈0.50) → a constant offset, no discrimination here. Magic-number; low practical impact.
- **`beat_loudness_band_ratio` is NULL even at L5** → the 10% beat-loudness term of the drums stem never contributes. Analyzer gap.
- **Reference-only / divergent constants** in `weights.py` and `config/transition.py`: `CAMELOT_BASE_SCORES` (`{1:0.95,2:0.85,…}`) differs from the live hard-coded locals in `neural_mix.py` (`{1:0.9,2:0.6,…}`); `config.transition.weight_*` and `CREST/LRA_DIFF_PENALTY_THRESHOLD` (4.0/5.0) are not consumed (live thresholds are config 10.0/8.0). These are now comment-marked as reference-only (kept because tests pin them as the spec).
