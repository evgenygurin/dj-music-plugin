...
### Поджанры как оси, а не папки

Техно-сеты редко строятся как строгая последовательность "один поджанр,
потом другой". Практичнее думать о нескольких независимых осях:

| Ось | Примеры | Что проверять в БД |
|---|---|---|
| Глубина / давление | dub, minimal, hypnotic, driving, peak, hard | `mood`, `energy_mean`, `integrated_lufs`, `spectral_centroid_hz` |
| Грув / сетка | Detroit, tribal, broken, psy-techno, industrial | `bpm`, `bpm_stability`, `onset_strength`, `energy_lowmid` |
| Тембр | raw, acid, EBM, dub, melodic | `hp_ratio`, `spectral_flux_*`, `spectral_centroid_hz`, Beatport metadata |
| Тональность | melodic, deep, dub, vocal vs atonal raw | `key_code`, `key_confidence`, `atonality`, `energy_mid/highmid` |
| Сценарий | warm-up, roller, peak, closing, handoff | template, position, target BPM/LUFS corridor |

Вывод для Claude Code: не пытаться угадать "жанр" одним полем. Сначала
выбрать intent и контекст, затем собрать подтверждающие признаки. Например,
raw/hypnotic материал может быть низко-мелодичным, но все равно держать
транс за счет 909/rumble, повторяющихся FM-sequences и постепенной
модуляции; это надо отличать от melodic/harmonic compatibility logic.
...
### DJ-приемы, которые полезны агенту

- **Klock / Dettmann school:** давление строится долго, через restrained
  EQ blends и смену металлической/дубовой текстуры, а не через быстрый
  parade of drops.
- **Dozzy / Mulero school:** hypnotic set держится micro-variation,
  пространством, низким novelty rate и аккуратным sub discipline.
- **Mills school:** короткие плотные tease/extract переходы могут быть
  лучше длинного blend, если треки machine-funk / Detroit-led.
- **Hawtin school:** sparse minimal работает, когда low-end и loop feel
  чистые; слишком много harmonic material ломает минималистскую сетку.
- **Lens / de Witte / peak festival school:** energy readable быстрее,
  переходы короче, drops яснее; но однотипный acid/peak wall надо
  разбавлять texture shifts.
- **Surgeon / Kraviz school:** raw cuts, resets и намеренная диссонансность
  допустимы как художественный прием; агент обязан назвать риск, а не
  выдавать их за "seamless mix".
...
### Большой пул / "огромная база"

1. Не отдавать весь каталог в `sequence_optimize`. Он reorder engine, а не
   selector.
2. Делать staged narrowing: hard filters (BPM/audio availability) ->
   style/feature filters -> diversity cap -> pair scoring -> final subset.
3. Проверять, что pool не стал слишком узким из-за NULL-фильтров или
   catch-all mood labels.
4. Если пользователь жалуется на однообразие, расширять candidate search
   по feature-similarity и Beatport metadata, а не только по `mood`.

### Плохие или неполные данные

1. Если `schema://entities/{entity}` расходится с prompt text, схема
   главнее.
2. Если `key_confidence` низкий или `atonality=true`, снижать вес Camelot
   и объяснять groove-first решение.
3. Если нет физического `audio_file`, нельзя обещать L5 analysis, точные
   cue points или готовую локальную выгрузку.
4. Если нет sections/beatgrid/downbeats, cheatsheet должен маркировать
   mix points как approximate.
5. Если Beatport genre и classifier `mood` конфликтуют, не перезаписывать
   одно другим без review; использовать конфликт как сигнал для аудита.
...
- **"Claude Code увидит новые hidden tools после unlock".** Не рассчитывать
  на это: часть клиентов кеширует tool list. В этом проекте tools видимы
  по умолчанию, а `tool_invoke` остается escape hatch.
...
## 11. Источники, проверенные 2026-07-05

- Beatport relaunch article, 2020: почему Techno был разделен на
  Peak Time/Driving/Hard и Raw/Deep/Hypnotic.
  https://www.beatportal.com/articles/440314-beatport-is-expanding-its-techno-categories
- Beatport Greenroom accepted genres, updated 2026-04-10: текущая delivery
  taxonomy для Techno (Peak Time / Driving), Techno (Raw / Deep / Hypnotic),
  Hard Techno и соседних жанров.
...
- Mixed In Key, phrasing and intensity / energy-flow workflow.
  https://mixedinkey.com/book/control-the-energy-level-of-your-dj-sets/
- DJ TechTools, phrase mixing fundamentals.
  https://djtechtools.com/2015/10/22/phrase-mixing-throwback-thursday-dj-technique/
- Sinee, raw/hypnotic techno sound-design reference: 909 drums, rumble,
  repeating/modulating sequences and cymbal energy.
  https://sinee.de/en/magazine/what-is-raw-hypnotic-techno-what-defines-the-underground-sub-genre-and-where-it-comes-from
- FastMCP 3.2.4 docs via Context7 + gofastmcp.com: tools, resources,
  prompts, CLI, structured output, annotations and visibility.
