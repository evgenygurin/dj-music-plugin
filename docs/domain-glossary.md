...
| **LUFS** | Loudness Units Full Scale — стандарт измерения громкости | `integrated_lufs: float`, обычно -20...-4 |
...
| **Stem** | Отдельный звуковой слой (vocals, drums, bass, other) | `separate_stems` tool (stub) |
...
| Термин | Описание | В коде |
|--------|----------|--------|
| **Transition** | Переход между двумя треками | `Transition` model, 6-компонентный score |
