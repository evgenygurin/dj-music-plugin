...
- @docs/tool-catalog.md — 20 generic tool dispatchers + 27 resources + 6 prompts
...
- @docs/vm-deployment.md — batch анализ на VM (deploy_to_vm.sh + vm_analyze.py)
- @docs/structure.md — файловая карта репозитория
- @docs/plugin-settings.md — per-project plugin config (.claude/dj-music.local.md)
- @docs/audio-schema.md — схема `track_audio_features_computed` (47 features)
...
- **Polymorphism over proliferation.** 6 generic CRUD tools (`entity_list/get/create/update/delete/aggregate`) + 3 provider tools (`provider_read/write/search`) + 2 compute (`transition_score_pool`, `sequence_optimize`) + `playlist_sync` + 2 admin (`unlock_namespace`, `tool_invoke`) + 6 UI (Prefab Apps) = **20 tool dispatchers**. Side-effects живут в handlers, привязанных к entity (create track = import, create audio_file = download, create track_features = analyze, create set_version = build+score).
...
```text
app/
├── tools/          # @tool — 20 dispatchers (6 entity + 3 provider + 2 compute + sync + 2 admin + 6 ui)
...
**v1.0.4** — документация sync с v1 surface. Удалён устаревший `scripts/vm_import_and_analyze.py` (сломался на Phase-7 cutover, зависел от `app.services.*` / `app.ym.*` / `app.controllers.*`). Удалён `REQUIREMENTS.md` (устаревшая спецификация времён v0). VM deployment переведён на одноразовый batch через `scripts/deploy_to_vm.sh` + `scripts/vm_analyze.py`. Continuous import+analyze loop на v1 surface пока не переписан. Актуальный tool surface: **20 dispatchers** (6 entity CRUD + 3 provider + 2 compute + 1 sync + 2 admin `unlock_namespace`/`tool_invoke` + 6 UI Prefab Apps) + 27 resources + 6 prompts + 6 handlers + 11 registered entities. Python-код без изменений.
...
**v1 surface:** **20 tool dispatchers** (6 entity CRUD + 3 provider + 2 compute + 1 sync + 2 admin [`unlock_namespace`, `tool_invoke`] + 6 UI [Prefab Apps, `app/tools/ui/`]) + **27 resources** (16 `local://`, 4 `schema://`, 3 `session://`, 4 `reference://`) + 6 prompts + 6 handlers. FastMCP v3 canonical layout (`tools/`, `resources/`, `prompts/`). Polymorphism через EntityRegistry (11 registered entities) + ProviderRegistry + handlers. Unit of Work + BaseRepository[M]. Pure domain (transition / optimization / camelot / template / audit). 18 audio analyzers, tiered L1-L4, section-aware scoring. 16 middleware.
