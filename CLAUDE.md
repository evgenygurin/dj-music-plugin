...
- **DI через `CurrentContext()`** — все `ctx` параметры в инструментах используют `ctx: Context = CurrentContext()` (не `Annotated[Context|None, ...] = None`)
- **Pydantic response models** — ключевые инструменты возвращают типизированные Pydantic модели из `app/schemas/tool_responses.py` (FastMCP авто-генерирует `output_schema`)
- **Tool transforms** — LLM-facing описания и hidden params настраиваются в `app/bootstrap/transforms.py` через `ToolTransform`, не в файлах инструментов
- **Per-session visibility** — `unlock_tools` использует `ctx.enable_components()` (не глобальный `server.enable()`)
...
- **Context injection:** `ctx: Context = CurrentContext()` + `# noqa: B008`. Не использовать `Annotated[Context|None, Field(...)] = None`
- **Prompts:** возвращают `PromptResult(messages=[...], description="...")`, не `list[Message]`
- **Resources:** возвращают `dict[str, Any]` (FastMCP сериализует в JSON). Исключение: knowledge:// блобы остаются `str` с `mime_type="application/json"`
- **Tool transforms:** скрывать технические параметры (`top_n`, `batch_size`, etc.) через `ArgTransformConfig(hide=True)` в `transforms.py`, а не в самом инструменте
