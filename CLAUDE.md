...
- REST API (`app/api/server.py`) — тонкая FastAPI обёртка для panel↔MCP
...
│   ├── dependencies.py     # Depends() factories + get_uow
│   └── middleware.py
├── api/               # Band 1 — FastAPI REST wrapper (server.py)
...
├── server.py          # FastMCP entry — composes lifespans, registers FSProvider
...
- `controllers → services → repositories → entities/db.models`
...
uv run lint-imports                        # Architecture contracts (4 contracts)
