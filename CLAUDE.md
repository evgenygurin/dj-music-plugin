...
- Plugin поднимает **два MCP сервера**: `mcp` (FastMCP, 50 DJ tools) + `db` (read-only инспекция БД, scoped к проекту)
- Panel (Next.js) — monitoring/analytics UI, reads from Supabase, mutations via MCP
- REST API (app/api/server.py) — тонкая FastAPI обёртка для panel→MCP коммуникации
...
app/api/server.py           → FastAPI REST API wrapper
```
