...
class ClippingRiskRule:
    ...
    def check(self, track_id: int, title: str, features: Any) -> list[AuditIssue]:
        if features.true_peak_db is None:
            return []
        if features.true_peak_db > settings.audit_true_peak_max:
            return [
                AuditIssue(
                    track_id=track_id,
                    title=title,
...
class UnreliableBpmRule:
    ...
    def check(self, track_id: int, title: str, features: Any) -> list[AuditIssue]:
        if features.bpm_confidence is None:
            return []
        if features.bpm_confidence < settings.audit_bpm_confidence_min:
            return [
                AuditIssue(
                    track_id=track_id,
                    title=title,
...
class UnreliableKeyRule:
    ...
    def check(self, track_id: int, title: str, features: Any) -> list[AuditIssue]:
        if features.key_confidence is None:
            return []
        if features.key_confidence < settings.audit_key_confidence_min:
            return [
                AuditIssue(
                    track_id=track_id,
                    title=title,
...
class TooHarmonicRule:
    ...
    def check(self, track_id: int, title: str, features: Any) -> list[AuditIssue]:
        if features.hp_ratio is None:
            return []
        if features.hp_ratio > settings.audit_hp_ratio_max:
            return [
                AuditIssue(
                    track_id=track_id,
                    title=title,
...
class ExcessiveDynamicsRule:
    ...
    def check(self, track_id: int, title: str, features: Any) -> list[AuditIssue]:
        if features.crest_factor_db is None:
            return []
        if features.crest_factor_db > settings.audit_crest_factor_max:
            return [
                AuditIssue(
                    track_id=track_id,
                    title=title,
...
class NoiseSpectrumRule:
    ...
    def check(self, track_id: int, title: str, features: Any) -> list[AuditIssue]:
        if features.spectral_flatness is None:
            return []
        if features.spectral_flatness > settings.audit_spectral_flatness_max:
            return [
                AuditIssue(
                    track_id=track_id,
                    title=title,
