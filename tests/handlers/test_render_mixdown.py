...
@pytest.mark.asyncio
async def test_mixdown_clamp_uses_inverse_tempo_ratio_for_fast_tracks(tmp_path, monkeypatch):
    RENDER_JOBS.clear()
    inputs = [
        TrackInput(
            track_id=0,
    ...
    monkeypatch.setattr(
        ...
        lambda p: type(
            "S",
            (),
            {"duration_s": 30.0, "true_peak_db": -1.4, "level_jumps": [], "near_silent_s": []},
        )(),
    )
    ...
    await render_mixdown_handler(
        ...
        out_name="MIX.mp3",
        body_bars=64,
        timestamp="20260706-000001",
    )

    target_bpm = get_settings().render.target_bpm
    bar_s = 4.0 * (60.0 / target_bpm)
    ratio = target_bpm / inputs[0].bpm
    expected_bars = int((60.0 / ratio) // bar_s)

    assert captured["body_bars"] == [expected_bars]
