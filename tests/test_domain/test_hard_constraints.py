...
class TestHardConstraints:
    def test_compatible_pair_passes(self) -> None:
        ...
        assert check_hard_constraints(a, b).rejection is None
    ...
    def test_bpm_diff_too_large_rejects(self) -> None:
        ...
        assert result.rejection.hard_reject is True
        assert "BPM" in (result.rejection.reject_reason or "")
    ...
    def test_camelot_distance_rejects_at_threshold(self) -> None:
        ...
        if result.rejection is not None:
            assert result.rejection.hard_reject is True
            assert "Camelot" in (result.rejection.reject_reason or "")
    ...
    def test_energy_gap_too_large_rejects(self) -> None:
        ...
        assert result.rejection is not None
        assert result.rejection.hard_reject is True
        assert "LUFS" in (result.rejection.reject_reason or "")
    ...
    def test_uses_settings_thresholds(self) -> None:
        ...
        assert check_hard_constraints(a, b).rejection is not None
