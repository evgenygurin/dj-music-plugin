"""Acceptance tests for the declarative set-commit flow."""
...
@pytest.mark.asyncio
async def test_commit_set_version_creates_version_and_transition_scores(
    acceptance_harness,
    patch_tiered_noop,
) -> None:
    ...
    built = await acceptance_harness.client.call_tool(
        "commit_set_version",
        {"track_ids": track_ids, "name": "Acceptance Commit Set"},
    )
