...
def test_execution_identity_changes_with_config() -> None:
    base = dict(source_hash="s", engine_version="e", analysis_version="a", model_version="m", dsp_version="d", renderer_version="r", seed=1)
    assert ExecutionIdentity(config_hash="one", **base).hash != ExecutionIdentity(config_hash="two", **base).hash
