"""Track and related models: Artist, Genre, Label, Release + junction tables."""
...
class Track(Base, TimestampMixin):
    """A music track in the local library."""
    ...
    track_labels: Mapped[list[TrackLabel]] = relationship(
        back_populates="track", cascade="all, delete-orphan"
    )
...
class Genre(Base, TimestampMixin):
    """A music genre with optional parent (hierarchical)."""
    ...
    parent: Mapped[Genre | None] = relationship(remote_side=[id], back_populates="children")
...
class TrackLabel(Base):
    ...
    track: Mapped[Track] = relationship(back_populates="track_labels")
    label: Mapped[Label] = relationship()


class TrackRelease(Base):
    """Track-Release association."""
...
class TrackExternalId(Base, TimestampMixin):
    """Track link to external platform ID."""

    __tablename__ = "track_external_ids"
    __table_args__ = (UniqueConstraint("track_id", "platform", name="uq_track_external_platform"),)
