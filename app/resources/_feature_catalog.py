...
STEM_FEATURE_CATALOG: dict[str, CatalogEntry] = {
    "track_id": {
        "group": "metadata",
        "label": "Track ID",
        "description": "Родительский трек. FK → tracks.id.",
    },
    "stem_name": {
        "group": "metadata",
        "label": "Stem",
        "description": "Тип стема: drums, bass, vocals, other или original.",
    },
    "analysis_level": {
        "group": "metadata",
        "label": "Analysis level",
        "description": "Уровень анализа (6 = L6 полный stem-анализ).",
    },
    "bpm": {
        "group": "tempo",
        "label": "BPM",
        "description": "Темп стема. Может отличаться от BPM полного трека.",
    },
    "bpm_confidence": {
        "group": "tempo",
        "label": "BPM confidence",
        "description": "Уверенность детекции BPM (0-1).",
    },
    "bpm_stability": {
        "group": "tempo",
        "label": "BPM stability",
        "description": "Стабильность темпа (0-1). Низкая = переменный темп.",
    },
    "variable_tempo": {
        "group": "tempo",
        "label": "Variable tempo",
        "description": "True если темп значительно меняется.",
    },
    "integrated_lufs": {
        "group": "loudness",
        "label": "Integrated LUFS",
        "description": "Средняя громкость стема по EBU R128. Ключевой параметр для energy budget.",
    },
    "short_term_lufs_mean": {
        "group": "loudness",
        "label": "Short-term LUFS mean",
        "description": "Средняя кратковременная громкость (3s окно).",
    },
    "momentary_max": {
        "group": "loudness",
        "label": "Max momentary LUFS",
        "description": "Пиковая momentary громкость.",
    },
    "rms_dbfs": {
        "group": "loudness",
        "label": "RMS dBFS",
        "description": "Среднеквадратичная амплитуда в dBFS.",
    },
    "true_peak_db": {
        "group": "loudness",
        "label": "True Peak dB",
        "description": "True peak (intersample). > 0 dBTP = клиппинг.",
    },
    "crest_factor_db": {
        "group": "loudness",
        "label": "Crest factor dB",
        "description": "Пик-фактор. Высокий = динамичный, низкий = сжатый.",
    },
    "loudness_range_lu": {
        "group": "loudness",
        "label": "Loudness range LU",
        "description": "Разброс громкости в LU.",
    },
    "energy_mean": {
        "group": "energy",
        "label": "Energy mean",
        "description": "Средняя энергия стема (нормализованная RMS).",
    },
    "energy_max": {
        "group": "energy",
        "label": "Energy max",
    ...
    "energy_std": {
        "group": "energy",
        "label": "Energy std",
        "description": "Стандартное отклонение энергии. Низкое = стабильная энергия (лупабельно).",
    },
    "energy_slope": {
        "group": "energy",
        "label": "Energy slope",
        "description": "Наклон энергии (рост/спад по времени).",
    },
    "energy_sub": {
        "group": "energy",
        "label": "Sub energy (20-60 Hz)",
        "description": "Энергия в sub-диапазоне. Кик + sub-bass.",
    },
    "energy_low": {
        "group": "energy",
        "label": "Low energy (60-250 Hz)",
        "description": "Энергия в low-диапазоне. Бас и нижняя середина кика.",
    },
    "energy_lowmid": {
        "group": "energy",
        "label": "Low-mid energy (250-500 Hz)",
        "description": "Энергия в low-mid. Теплота, тело звука.",
    },
    "energy_mid": {
        "group": "energy",
        "label": "Mid energy (500-2000 Hz)",
        "description": "Энергия в mid. Основная читаемость, синтезаторы.",
    },
    "energy_highmid": {
        "group": "energy",
        "label": "High-mid energy (2-4 kHz)",
        "description": "Энергия в high-mid. Атака, присутствие, хэты.",
    },
    "energy_high": {
        "group": "energy",
        "label": "High energy (4-8 kHz)",
        "description": "Энергия в high. Воздух, шлейфы, shimmer.",
    },
    "spectral_centroid_hz": {
        "group": "spectral",
        "label": "Spectral centroid Hz",
        "description": "Центр тяжести спектра. Высокий = яркий, низкий = тёмный.",
    },
    "spectral_rolloff_85": {
        "group": "spectral",
        "label": "Rolloff 85%",
        "description": "Частота ниже которой 85% энергии спектра.",
    },
    "spectral_rolloff_95": {
        "group": "spectral",
        "label": "Rolloff 95%",
        "description": "Частота ниже которой 95% энергии спектра.",
    },
    "spectral_contrast": {
        "group": "spectral",
        "label": "Spectral contrast",
        "description": "Контраст пик-провал по октавам.",
    },
    "key_code": {
        "group": "key",
        "label": "Key code",
        "description": "Camelot-код стема (0-23). 0=8B, 23=1B.",
    },
    "key_confidence": {
        "group": "key",
        "label": "Key confidence",
        "description": "Уверенность определения ключа (0-1).",
    },
    "hnr_db": {
        "group": "key",
        "label": "HNR dB",
        "description": "Harmonics-to-noise ratio. Высокий = гармоничный, низкий = шумный.",
    },
    "chroma_entropy": {
        "group": "key",
        "label": "Chroma entropy",
        "description": "Энтропия хромаграммы. Высокая = много нот, низкая = одна нота/аккорд.",
    },
    "onset_rate": {
        "group": "rhythm",
        "label": "Onset rate",
        "description": "Плотность атак (ударов) в секунду.",
    },
    "pulse_clarity": {
        "group": "rhythm",
        "label": "Pulse clarity",
        "description": "Чёткость пульсации (0-1). Высокая = ровный бит.",
    },
    "kick_prominence": {
        "group": "rhythm",
        "label": "Kick prominence",
        "description": "Выраженность кика (0-1). Высокая = мощный, читаемый кик.",
    },
    "hp_ratio": {
        "group": "rhythm",
        "label": "HP ratio",
        "description": "Доля высокочастотной перкуссии.",
    },
    "danceability": {
        "group": "danceability",
        "label": "Danceability",
        "description": "Танцевальность (0-1).",
    },
    "dissonance_mean": {
        "group": "danceability",
        "label": "Dissonance mean",
        "description": "Средний диссонанс. >0.3 = резкое, индустриальное звучание.",
    },
    "chords_strength": {
        "group": "L6",
        "label": "Chords strength",
        "description": "Сила аккордовой структуры (Essentia chords).",
    },
    "chords_changes_rate": {
        "group": "L6",
        "label": "Chords changes rate",
        "description": "Частота смены аккордов.",
    },
    "inharmonicity": {
        "group": "L6",
        "label": "Inharmonicity",
        "description": "Негармоничность спектра. Высокая = колокольность, металличность.",
    },
    "click_detected": {
        "group": "L6",
        "label": "Click detected",
        "description": "Обнаружены ли щелчки/клиппинг в стеме.",
    },
    "saturation_detected": {
        "group": "L6",
        "label": "Saturation detected",
        "description": "Обнаружено ли насыщение/сатурация.",
    },
    "drum_bands": {
        "group": "L6",
        "label": "Drum bands",
        "description": "JSONB: sub_kick, kick_body, snare_clap, hi_hats — энергия и onset_rate per band.",
    },
}
