...
def validate_stem(
    ...
    *,
    ...
    try:
        ...
        if (
            check_duration
            and abs(info.frames / info.samplerate - source.duration) > DURATION_TOLERANCE_SECONDS
        ):
            return StemValidationResult(
                False,
                str(path),
    ...
    if not np.isfinite(data).all():
        return StemValidationResult(
            False, str(path), info.samplerate, info.channels, info.frames, reason="non_finite"
        )
    rms = float(np.sqrt(np.mean(np.square(data), dtype=np.float64))) if data.size else 0.0
    peak = float(np.max(np.abs(data))) if data.size else 0.0
    if reject_all_zero and peak == 0.0:
        return StemValidationResult(
            False, str(path), info.samplerate, info.channels, info.frames, rms, peak, "all_zero"
        )
    return StemValidationResult(
        True, str(path), info.samplerate, info.channels, info.frames, rms, peak
    )
