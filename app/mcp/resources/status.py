...
async def library_status(
    ...
    active_result = await session.execute(
        select(func.count(Track.id)).where(Track.status == 0)
    )
    ...
    # Calculate coverage
    coverage = (
        (tracks_with_features / total_tracks * 100) if total_tracks > 0 else 0.0
    )
...
async def platforms_status(
    ...
    # Yandex Music
    ym_count_result = await session.execute(
        select(func.count(YandexMetadata.track_id))
    )
    ...
    # Spotify
    spotify_count_result = await session.execute(
        select(func.count(SpotifyMetadata.track_id))
    )
    ...
    # Beatport
    beatport_count_result = await session.execute(
        select(func.count(BeatportMetadata.track_id))
    )
    ...
    # SoundCloud
    sc_count_result = await session.execute(
        select(func.count(SoundCloudMetadata.track_id))
    )
