...
async def track_features(
    ...
    if features.key_code is not None:
        camelot_notation, key_full_name = CAMELOT_KEYS.get(
            features.key_code, ("?", "Unknown")
        )
...
async def set_summary(
    ...
    track_count_result = await session.execute(
        select(func.count()).where(
            DJSetVersion.id == latest_version.id  # TODO: fix when SetItem model exists
        )
    )
...
async def catalog_stats(
    ...
    avg_energy_result = await session.execute(
        select(
            func.avg(TrackAudioFeaturesComputed.energy_lufs_integrated)
